...
# Import statewide hydro power generation data (1989 - 2020) for forecasting
...
def smooth2(x_future, y_data, alpha, beta): # takes "array-like" data let's try a pandas series.
    # create model
...
def renewable_forecast(state, solar_gen, wind_gen, hydro_gen, geothermal_gen):
    future_years = future_years = list(range(2020,2051))  # Data sets in this function go from 1989-2020, predictions go from 2020-2050
    ...
    yhat_hydro = smooth2(future_years, hydro_gen[state], 0.1, 0) # alpha = 0.1 ignores yearly fluctuations and focuses on historical trends.
...
def renewable_fraction_forecast(state, historical_total, r_forecast_df): # predict Fraction or renewables generated from 2020 - 2050
    ...
    future_years = future_years = list(range(2020,2051))
...
def get_renewable_goals(state, state_goals): # takes imported df of state_goals
    ...
    if state_goals['state'].isin([state]).any(): # If the state_goals df contains the selected state
        ...
        sg_clean = sg[sg['goal'].notna()]  # eliminate nan values. only plot if has_goals == True
...
with col2:
    ...
    'Predicted Renewable Generation': renewable_forecast(state, solar_gen, wind_gen, hydro_gen, geothermal_gen) })
    ...
    renewable_fraction_chart = (
                ...
                .encode(
                    x=alt.X('year', title='year'),
                    y=alt.Y(state, title=' % Power Produced From Renewables' , scale=alt.Scale(domain=[0, 100])),       # y = alt.Y('count()', scale=alt.Scale(domain=[0, 120]),
                    #color = alt.Color('blue')
                )
                #.mark_rule(color='red').encode(y=alt.datum(1))
                #.mark_rule(color='red').encode(y=alt.datum(state_potential_dict[state]['solar_potential']))
    ...
    'Predicted Percent Power From Renewables': 100.0 * renewable_fraction_forecast(state, historical_total, 
        renewable_forecast(state, solar_gen, wind_gen, hydro_gen, geothermal_gen)) }) # multiply by 100.0 to go from fraction to %
